...
class FlashAttnFunc(torch.autograd.Function):
    @staticmethod
    def forward(ctx, query, key, value, bias, scale=None, p=0.0):
        """
        ...
        mem_eff_attn_op = AttentionOpDispatch.from_arguments(
            query=query, key=key, value=value, attn_bias=bias, scale=scale, p=p
        ).op

        out, lse = memory_efficient_attention_forward_requires_grad(
            query, key, value, op=mem_eff_attn_op[0], attn_bias=bias, scale=scale, p=p
        )
        # out, lse = memory_efficient_attention_forward_requires_grad(
